...
if __name__ == "__main__":
    ...
    image = MoveableImage('test', imageProperties.heartyellow, [5, 10], [2, 6])
    # image = MoveableImage('test', [[[50, 255, 255] for i in range(6)] for i in range(10)], [5, 10], [2, 6])
    ...
    print('\ngetMatrixPosition (Expected: [[5, 10][2, 6]]): ', image.getMatrixPosition())   # 9
    print('\ngetXvalues (Expected: [5, 10]): ', image.getXvalues())                         # 10
    print('\ngetYvalues (Expected: [2, 6]): ', image.getYvalues())                          # 11
    ...
    time.sleep(2)
    send.run(image.getLightOffImage())                                  # 13
    image.setLightOffImage(imageProperties.heartblue)                    # 14
    time.sleep(2)
    send.run(image.getLightOffImage())                                  # 13
    image.resetLightOffImage()                                          # 16
    time.sleep(2)
    send.run(image.getLightOffImage())                                  # 13
    image.setInputImage(imageProperties.heartgreen, 'landscape')             # 18
    time.sleep(2)
    send.run(image.getImage())                                          # 12
    time.sleep(1)
